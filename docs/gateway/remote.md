...
- For **operators (you / the browser dashboard)**: SSH tunneling is the universal fallback.
...
- Forward `18789` over SSH or Tailscale Serve.
- Connect with the Control UI or WebChat over the forwarded loopback endpoint.

Runbook: [Windows (WSL2)](/platforms/windows).
...
The browser dashboard can drive the same setup end-to-end once the SSH tunnel is up (remote status checks, WebChat, and health probes).
