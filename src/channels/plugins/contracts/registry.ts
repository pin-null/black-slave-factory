...
const MATRIX_SESSION_BINDING_ACCOUNT_ID = "ops";
const MATRIX_SESSION_BINDING_CONVERSATION_ID = "!room:example";
const MATRIX_SESSION_BINDING_AUTH = {
  accountId: MATRIX_SESSION_BINDING_ACCOUNT_ID,
...
let matrixSessionBindingManager: { stop: () => void } | null = null;

async function ensureMatrixSessionBindingManager() {
  if (!matrixSessionBindingStateDir) {
    matrixSessionBindingStateDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "openclaw-matrix-binding-"),
    );
  }
  setMatrixRuntime({
    state: {
  ...
  matrixSessionBindingManager = await createMatrixThreadBindingManager({
    accountId: MATRIX_SESSION_BINDING_ACCOUNT_ID,
    auth: MATRIX_SESSION_BINDING_AUTH,
    client: {} as never,
...
function cleanupMatrixSessionBindingFixture() {
  matrixSessionBindingManager?.stop();
  matrixSessionBindingManager = null;
  if (matrixSessionBindingStateDir) {
    fs.rmSync(matrixSessionBindingStateDir, { recursive: true, force: true });
    matrixSessionBindingStateDir = null;
  }
}
...
export const sessionBindingContractRegistry: SessionBindingContractEntry[] = [
  ...
  },
  {
    ...
    bindAndResolve: async () => {
      ...
      const binding = await service.bind({
        ...
        conversation: {
          channel: "matrix",
          accountId: MATRIX_SESSION_BINDING_ACCOUNT_ID,
          conversationId: MATRIX_SESSION_BINDING_CONVERSATION_ID,
        },
        placement: "current",
        metadata: {
          label: "codex-matrix",
    ...
    unbindAndVerify: unbindAndExpectClearedSessionBinding,
    cleanup: async () => {
      cleanupMatrixSessionBindingFixture();
      expectClearedSessionBinding({
        channel: "matrix",
        accountId: MATRIX_SESSION_BINDING_ACCOUNT_ID,
        conversationId: MATRIX_SESSION_BINDING_CONVERSATION_ID,
